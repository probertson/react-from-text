...
function fromText(message, placeholders) {
  ...
  var sprintfed = (0, _sprintfJs.sprintf)(message, placeholders);
  var ast = parseFragment(sprintfed, context);
...
function parseFragment(rawMessage, context) {
  ...
  while (message !== '') {
    var _parseNext = parseNext(message, context),
        ...
        props = _objectWithoutProperties(_parseNext, ["remaining"]);
...
function substituteTags(message, placeholders, context) {
  return message.map(function (node, i) {
    if (node.kind === NODES.TAG) {
      ...
      (0, _invariant["default"])(placeholders.hasOwnProperty(name), createError("missing placeholder value <".concat(name, ">"), context));
      var props = {
