...
exports["default"] = reactFromText;

var _react = _interopRequireDefault(require("react"));

var _sprintfJs = require("sprintf-js");

var _baseFormatChildren = _interopRequireDefault(require("./base-format-children"));
 ...
 */
function reactFromText(message, placeholders) {
  return replacePlaceholders((0, _baseFormatChildren["default"])(applyChildren, message, placeholders), placeholders);
}

function replacePlaceholders(segment, placeholders, dedupeKeySuffix) {
  // it's a string so just substitute the placeholders
  if (typeof segment === 'string') {
    return (0, _sprintfJs.sprintf)(segment, placeholders);
  } // it's an array of (strings and/or React elements) so perform replacement on each one


  if (Array.isArray(segment)) {
    return segment.map(function (subSegment, index) {
      return replacePlaceholders(subSegment, placeholders, index.toString());
    });
  } // it's a React element, so perform replacement on its child(ren) instead


  if (segment.props.children) {
    var replacedChildren = replacePlaceholders(segment.props.children, placeholders);
    return _react["default"].cloneElement.apply(_react["default"], [segment, {
  ...
  return dedupeKeySuffix ? _react["default"].cloneElement.apply(_react["default"], [segment, {
    key: segment.key + dedupeKeySuffix
  }]) : segment;
}

function applyChildren(key, element, children) {
  if (process.env.NODE_ENV !== 'production' && !_react["default"].isValidElement(element)) {
    throw new Error(JSON.stringify(element) + ' is not a valid element');
  }

  return _react["default"].cloneElement.apply(_react["default"], [element, {
    key: element.key || key
  }].concat(children || []));
}